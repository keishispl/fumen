...
function parseSong(song, genre) {
     ...
     Object.entries(song).forEach(([key, value]) => {
          ...
          // If the key is not "chart", center the text.
          if (key !== "chart") cell.style.cssText = "text-align:center;";
          ...
          switch (key) {
               case "song":
                    ...
                    if (song.link) {
                         cell.innerHTML = `<a href="https://www.youtube.com/watch?v=${song.link}" class="hiddenlink" target="_blank">${value.join(`<br>`)}</a>`;
                    } else {
               ...
               case "chart":
                    ...
                    cell.innerHTML = difficulties.map(difficulty => {
                         if (value[difficulty.name]) {
                              ...
                              return `<div><img class="icon" src="${difficulty.icon}" draggable="false">: ☆${value[difficulty.name].join(` / `)} コンボ</div>`;
                         } else {
                              if (difficulty.name !== "ura") {
                                   // If the difficulty is not specified, add a dash to the table.
                                   return `<div><img class="icon" src="${difficulty.icon}" draggable="false">: -</div>`;
                              }
                         }
                    }).join("");
                    break;
               case "":
                    ...
                    if (value === "" || value === "true") {
                         ...
                         cell.innerHTML = `<a href="https://drive.google.com/drive/folders/${value}/" class="hiddenlink" target="_blank"><i class="fa-solid fa-download"></i></a>`;
                    }
                    break;
               default:
